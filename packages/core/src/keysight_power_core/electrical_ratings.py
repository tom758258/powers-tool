...
ELECTRICAL_RATINGS_BY_MODEL: Mapping[str, ModelElectricalRatings] = MappingProxyType(
    {
        E36312A_ELECTRICAL_RATINGS.model: E36312A_ELECTRICAL_RATINGS,
        EDU36311A_ELECTRICAL_RATINGS.model: EDU36311A_ELECTRICAL_RATINGS,
    }
)


def ratings_for_model(model: str | None) -> ModelElectricalRatings | None:
    if not model:
        return None
    return ELECTRICAL_RATINGS_BY_MODEL.get(model.upper())


def electrical_ratings_by_model_metadata() -> dict[str, dict[str, object]]:
    return {model: ratings.to_dict() for model, ratings in ELECTRICAL_RATINGS_BY_MODEL.items()}
