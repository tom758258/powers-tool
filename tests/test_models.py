from keysight_power.models import (
    REGISTERED_MODELS,
    lookup_model,
    parse_idn,
    resource_interface,
...
def test_parse_standard_keysight_idn() -> None:
    idn = parse_idn("KEYSIGHT,E36312A,MY12345678,1.2.3")

    assert idn.raw == "KEYSIGHT,E36312A,MY12345678,1.2.3"
    ...
    assert idn.to_dict() == {
        "raw": "KEYSIGHT,E36312A,MY12345678,1.2.3",
...
def test_parse_idn_trims_fields() -> None:
    idn = parse_idn(" KEYSIGHT , E36103B , MY00000000 , 1.0 ")

    assert idn.manufacturer == "KEYSIGHT"
    assert idn.model == "E36103B"
    assert idn.serial == "MY00000000"
...
def test_parse_idn_preserves_extra_commas_in_firmware() -> None:
    idn = parse_idn("KEYSIGHT,E36312A,MY12345678,1.2.3,build 4")
