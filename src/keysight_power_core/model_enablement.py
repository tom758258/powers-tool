...
class ModelEnablementInventory:
    ...
    output_limit_models: frozenset[str]
...
def current_model_enablement_inventory() -> ModelEnablementInventory:
    ...
    setpoint_models = frozenset(SETPOINT_RANGES_BY_MODEL)
    return ModelEnablementInventory(
        ...
        command_capabilities={model: command_support(model) for model in active_or_candidate},
        live_policies={policy.model: policy for policy in LIVE_SUPPORT_POLICY_REGISTRY},
        output_limit_models=frozenset(ELECTRICAL_RATINGS_BY_MODEL) | setpoint_models,
        setpoint_range_models=setpoint_models,
...
def _validate_enabled_model(
    ...
    *,
    ...
    if output_capable:
        if model not in inventory.output_limit_models:
            raise ValueError(f"{model}: missing electrical rating or range-dependent output limits")
