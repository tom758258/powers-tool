...
SETPOINT_RANGES_BY_MODEL: Mapping[str, ModelSetpointRanges] = MappingProxyType(
    {
        E36312A_SETPOINT_RANGES.model: E36312A_SETPOINT_RANGES,
        EDU36311A_SETPOINT_RANGES.model: EDU36311A_SETPOINT_RANGES,
        E3646A_SETPOINT_RANGES.model: E3646A_SETPOINT_RANGES,
    }
)


def setpoint_ranges_for_model(model: str | None) -> ModelSetpointRanges | None:
    if not model:
        return None
    return SETPOINT_RANGES_BY_MODEL.get(model.upper())


def setpoint_ranges_by_model_metadata() -> dict[str, dict[str, object]]:
    return {model: ranges.to_dict() for model, ranges in SETPOINT_RANGES_BY_MODEL.items()}
