export function isNoHardwareExecutionMode(mode) {
  return mode === "simulate" || mode === "dry-run";
}

export function buildWorkspaceResultKey(context) {
...
export function buildWorkspaceResultContextForJob(job, modelMaps = {}) {
  ...
  return { command: job.command, executionMode, resource, expectedModelGuard: runtime.expected_model_id || "", canonicalModelId };
}

export function buildCurrentWorkspaceResultContext(snapshot) {
  const { command, executionMode, planningIdentity = "", resource = "", expectedModelGuard = "", canonicalModelId = "" } = snapshot;
  if (executionMode === "simulate") return { command, executionMode, planningModelId: planningIdentity || "" };
  if (executionMode === "dry-run") {
  ...
  return { command, executionMode: "real", resource, expectedModelGuard, canonicalModelId };
}

export function buildWorkspaceResultEntry(job, modelMaps = {}) {
  ...
  return { context, key: buildWorkspaceResultKey(context), job };
}

export function findWorkspaceResult(workspaceResults, context) {
